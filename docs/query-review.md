...
## รอบ 2 (re-check เฉพาะ 2 จุด) — ผ่านทั้งคู่

revision ที่ตรวจ (working tree หลังแก้ · **ยังไม่ commit**):
`src/db/queries/transactions.ts` md5 **`dd3ce6aaaf3d9b231287a27ccef41aab`** ·
`src/db/queries/transactions.test.ts` md5 **`b63f7f0261dfa8455e3bc05fb03fd5b3`**
· harness พิมพ์ md5 ก่อนและหลังรัน → **เท่ากันทั้งสองครั้ง** (ไม่มี edit ระหว่างตรวจ)

คำสั่งเดียวที่ใช้ตรวจซ้ำ: `node /tmp/probe-query/recheck.mjs`
(harness มี 4 ส่วนตาม 2 จุดที่ตกลง: ชุดเทสต์จริง · mutation 2 แบบบนสำเนา `/tmp/mutq2` · tie fixture ยิงโค้ดจริง · wildcard probe)

| ตรวจ | ผล |
|---|---|
| ชุดเทสต์จริง | tests **10** · pass **10** · fail 0 (เดิม 8 → +2: keyset tie-break, escape) |
| M-tie: ถอด tie-break ออก (เหลือ `lt(occurredAt)`) | **จับได้** — fail 2 (`keyset: แถวที่ occurred_at เท่ากันเป๊ะ…` + `หน้าแรก: เรียงใหม่->เก่า และ keyset ต่อหน้าไม่ซ้ำ/ไม่ข้าม`) |
| M-escape: ถอด `escapeLike()` ออก | **จับได้** — fail 1 (`ค้นหา: escape อักขระพิเศษของ LIKE`) |
| tie fixture ของรีวิว (4+3+1 แถว · limit 2 · โค้ดจริง) | 8/8 แถว · id ไม่ซ้ำ 8 · เรียง desc ตาม `(occurredAt, id)` จริง · cursor ที่ id เดิมถูกตัดออก (ไม่ค้าง) |
| wildcard (โค้ดจริง) | `a_b` → 1 แถว (ก่อนแก้ 2: `a_b`+`axb`) · `%` → 1 แถว (ก่อนแก้ 12) · `50%` → 1 แถว ✓ |
| เทสต์ใหม่มี fixture guard | ✓ (assert ว่ามีกลุ่มเวลาเท่ากัน ≥3 อย่างน้อย 2 กลุ่ม และมีแถวในกลุ่มเวลาซ้ำ ≥10 — กันเทสต์ผ่านแบบกลวง) + cross-check กับ query เดียวไม่แบ่งหน้า (`limit 1000`) |
| escape ครอบ `\` ด้วย | ✓ เกินที่ผมเสนอเล็กน้อยและถูกต้อง — เทสต์ยืนยันว่าค้น `\` ไม่ทำให้ PG error (ได้ 0 แถว) |

**ข้อ 3 (keyset) ปิดแล้ว** — โค้ดเดิมถูกอยู่แล้ว ตอนนี้มีเทสต์ล็อกพฤติกรรมจริง (ถอด tie-break แล้วล้ม)
**ข้อ 4 (escape) ปิดแล้ว** — ผลค้นหาถูกต้องตามที่วัด และมีเทสต์กันถอยหลัง

**บทเรียนของ harness (แก้ไว้แล้วในสคริปต์):** รอบแรกของผมรันแล้วได้ผลขัดกันเอง (mutation ไม่ถูกจับ แต่ probe กลับเห็น escape ทำงาน)
เพราะ coder เซฟไฟล์อยู่ระหว่างการรัน — harness จึงพิมพ์ md5 ก่อน/หลัง และผมถือว่าผลใช้ได้เฉพาะเมื่อ md5 เท่ากัน

ไม่เปิดประเด็นใหม่ · ข้อ 5 (ระดับหลักฐาน index) lead รับตามที่จัดชั้นไว้ (structural/compatibility) — ไม่มี action
